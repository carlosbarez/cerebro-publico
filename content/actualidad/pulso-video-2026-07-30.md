...
> Digest de **12 vídeos, 6 canales activos de 8**, ventana 29-30 jul: un canal técnico español 1, una mesa de mercado española 2 (Cierre +
> Media sesión del 29-jul, Apertura descartada por solapar con el Cierre ya con la Fed decidida), un divulgador macro español 1,
> una cadena económica española 4 (noticiero 30-jul + temática Wall Street + entrevistas Beltrán/oro y Paco Arnao/Irán),
> una cadena financiera internacional 3 (`Open Interest` Meta/Microsoft + `Fed Special` + clip Kospi/SK Hynix, todas 29-jul,
> jornada D-1), una cadena financiera internacional 1 (fuente externa). **un analista macro español-Bolsa
> sin vídeo nuevo; un gestor de fondos español re-descubierto por `--dateafter` es el mismo vídeo del oro ya procesado el
> 29-jul** (mismo ID, descartado sin reprocesar).
>
> **Aviso de proceso**: 3 de 12 destilados (fuente externa) volvieron a caer en el proveedor primario de OmniRoute (`insufficient_quota`) y se resolvieron por
...
> efímero). El propio noticiero de una cadena económica española (30-jul) ya marca sin verificar dos afirmaciones israelíes
> (Mojtaba Jamenei con vida; Irán con ~1.500 misiles balísticos) — se respeta esa cautela, no se eleva a
> hecho.
>
> **Descartes de volumen**: una cadena económica española (25 candidatos listados, la mayoría flashes redundantes sobre el mismo
...
> noticiero-gemelo del 29-jul (mismo guion, se queda el del 30-jul más reciente). una cadena financiera internacional (~55 clips
> restantes de 60): `Balance of Power`, `una cadena financiera internacional Crypto`, entrevistas sin cartera/watchlist (UBS, Logitech,
> Stonepeak, Coinbase). una cadena financiera internacional: los dos "una cadena financiera internacional Talks" de Earnings Roundup/Instant Reaction
> son el mismo panel reetiquetado en 5 shows distintos (Surveillance/Tech/Businessweek/Daybreak/Intelligence)
> — contenido ya cubierto por `Open Interest`/`Fed Special`, dedup. una mesa de mercado española Apertura (pre-Fed) descartada:
...
- **El dato**: FOMC mantiene el tipo de referencia en 3,50-3,75% con **9 votos a favor, 3 en contra**
  (tres presidentes de Fed regionales pidiendo subida — una cadena financiera internacional `Fed Special` cuenta "nine versus three"
  sin dar nombres; el pulso de texto de la ingesta de texto, [[pulso-2026-07-30]], los identifica: Beth Hammack
  (Cleveland), Neel Kashkari (Minneapolis), Lorrie Logan (Dallas), corroboración cruzada entre las dos rutinas
  del cerebro). Kevin Warsh (nuevo Fed Chair) dio una rueda de prensa que el propio panel de una cadena financiera internacional
  ...
  **6,70%**. Probabilidad de subida en septiembre cae de 70% a **50%** según los futuros de fondos federales
  citados por una cadena financiera internacional. una mesa de mercado española (Cierre, 29-jul) confirma el mismo patrón desde el lado de renta variable:
  S&P 500 −1,73% a **7.389** ("ojo con 7.400, que es bastante peligroso" — literal), Nasdaq −1,50% a 27.490,
  Russell −1,6% a 2.921 (fuente externa).
- **Mecanismo**: el mercado no está reaccionando a la decisión (ampliamente esperada, hold) sino a que Warsh
  ...
  panelista de una cadena financiera internacional. Eso es lo que mueve el extremo largo de la curva (30 años) mucho más que el corto
  (2 años, −6pb): el mercado exige una prima por incertidumbre de gobernanza de la Fed, no solo por
  inflación. Encaja con el patrón de "farol desafiado" que ya describió una mesa de mercado española ayer (30Y +10pb al cierre).
...
- 📌 **predicción** (fuente externa): probabilidad de subida en
...
- **El dato**: Irán lanzó **misiles balísticos contra una base aérea estadounidense en Jordania**
  (fuente externa); EEUU respondió con ataques contra posiciones respaldadas por Irán en Irak, **con Arabia Saudí
  uniéndose al ataque** (fuente externa).
  Un proyectil alcanzó además la provincia iraní de Azerbaiyán Occidental (fuente externa). Ataque adicional con dron a una instalación flotante de GNL en el puerto egipcio
...
- **Hilo Ucrania-Irán, actualización**: Paco Arnao (fuente externa) confirma el mecanismo del hilo que se reabrió el 29-jul:
...
- **Mecanismo**: la escalada convierte lo que llevaba runs siendo "amenaza sin confirmar" en **guerra directa
  ...
  estrechos a la vez (fuente externa). El premio de riesgo geopolítico en el petróleo (ver niveles abajo) es la traducción directa a
  mercado.
- **Cruce**: [[mineria-industrial-y-energia]] · [[ciclos-de-mercado]].
- **Conflicto de interés / encuadre**: el senador John Kennedy (fuente externa) respalda explícitamente la
...
- **El dato**: SK Hynix reportó un salto fuerte en beneficio operativo (fuente externa) pero fue un **"clean miss" frente al consenso**
  ...
  **sostener márgenes del 80% o más** en memoria — eso, no la cifra absoluta, es lo que mueve el **swing
  intradía del 15% en el Kospi** (fuente externa). una mesa de mercado española (Cierre, 29-jul) da el cierre: Kospi −9% (con mínimo
  intradía de −13/−14%) — cifras compatibles con el "swing del 15%" de una cadena financiera internacional si se mide pico a valle, no
  ...
  vez en este hilo como riesgo estructural adicional, según el analista de una cadena financiera internacional — no solo el ciclo de
  demanda.
- **Mecanismo**: el hilo abierto desde hace 11 runs (fuente externa) suma un dato nuevo y más preciso: el mercado ya no discute si SK Hynix gana dinero (lo hace,
...
- 📌 **predicción** (fuente externa): SK Hynix mantiene márgenes ≥80% en memoria pese a la
...
- **El dato**: **Meta** — ingresos $60.800M (+28% interanual) pero **por debajo de estimaciones**; BPA $6,18
  frente a estimación de $7,19 y por debajo del año anterior. Guía previa de consenso citada por una cadena económica española:
  $58.000-61.000M (Meta cerró dentro del rango, en el extremo alto, pero el BPA decepcionó). **Microsoft** —
  ingresos $90.010M (+18% interanual), **por encima de estimaciones** (fuente externa); BPA
  diluido $4,74 (+23% interanual). Azure (cloud) creciendo **>40%** en el trimestre. El dato más denso del
  bloque, vía una cadena financiera internacional `Open Interest`: el consenso de mercado ("the Street") espera que Microsoft guíe el
...
- **Mecanismo**: es la misma tesis de fondo que un gestor de fondos español planteó ayer con escepticismo explícito sobre la
  revalorización de reservas de oro de EEUU ([[pulso-video-2026-07-29]], hilo #5) — hoy la repite una fuente
  distinta (fuente externa), pero **de mucha menor autoridad** (educador de
...
- **Micron/memoria** (cartera, posición más concentrada, +690%): el hilo un divulgador macro español-vs-una mesa de mercado española sigue sin
...
5. **Deuda/déficit de EEUU** (arrastrado desde el 29-jul, `[Sin verificar]`): sigue pendiente de contrastar
   contra fuente oficial (Tesoro/FRED) — hoy son ya dos fuentes de vídeo (fuente externa) que se
   apoyan en la narrativa sin dar la cifra exacta verificada.
6. **Apple/Amazon reportan hoy (30-jul)**: fuera de la ventana de una cadena financiera internacional de este run (jornada D-1);
