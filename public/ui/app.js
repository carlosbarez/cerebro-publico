...
(() => {
  ...
  // internet: SOLO las que vienen de groundingMetadata (pregunta.js las filtra); enlace externo nuevo.
  ...
  const SECCIONES = ['chat', 'newsletter', 'cartera', 'biblioteca'];
  ...
  const activarSeccion = (nombre) => {
    ...
    const cargas = { cartera: pintarCartera, newsletter: pintarNewsletter, biblioteca: pintarBiblioteca };
  ...
  const guardarCartera = (posiciones) => {
    try { localStorage.setItem(CLAVE_CARTERA, JSON.stringify(posiciones)); } catch (e) {
      ...
      $('#cartera-vacia').textContent = 'No se pudo guardar en este navegador (¿modo privado?). La lista dura hasta que recargues.';
      $('#cartera-vacia').hidden = false;
    }
  };
  const numeroODefecto = (v) => { const n = Number.parseFloat(v); return Number.isFinite(n) && n > 0 ? n : null; };

  // Porcentaje con coma (es-ES) para los pesos por bloque.
  const _pct = (n) => (Number.isFinite(n) ? n.toFixed(1).replace('.', ',') : 'n/d');
  const pintaPesos = (pesos) => {
    const el = $('#cartera-pesos');
    el.replaceChildren();
    const divs = Object.keys(pesos.divisas).sort();
    if (!divs.length) { el.hidden = true; return; }
    el.hidden = false;
    divs.forEach((div) => {
      el.append(crear('h3', divs.length > 1 ? `Peso por bloque (${div})` : 'Peso por bloque'));
      const ul = crear('ul', undefined, 'pesos-bloque');
      Object.keys(pesos.divisas[div].pesos).sort().forEach((b) => {
        const li = crear('li');
        li.append(crear('span', b.replace(/-/g, ' '), 'bloque'));
        li.append(crear('span', `${_pct(pesos.divisas[div].pesos[b])} %`, 'peso'));
        ul.append(li);
      });
      el.append(ul);
    });
    if (pesos.sinPrecio) el.append(crear('p', `${pesos.sinPrecio} posición(es) sin precio no pesan.`, 'meta'));
  };
  const pegarCartera = () => {
    const { posiciones, ilegibles, sinBloque } = window.CerebroCartera.parseaCartera($('#cartera-pegar').value);
    ...
    if (sinBloque) partes.push(`${sinBloque} línea(s) con bloque desconocido fueron a «sin clasificar»`);
  ...
  const pintarCartera = async () => {
    ...
    const r = await fetch(`/api/cotiza?simbolos=${encodeURIComponent(posiciones.map((p) => p.ticker).join(','))}`)
      .then((x) => x.json())
      .catch(() => ({ cotizaciones: {}, trazas: ['no se pudo contactar con el servidor de precios'] }));
    finEsqueleto(lista);
    lista.replaceChildren();
    ...
    posiciones.forEach((p, i) => {
      const dato = (r.cotizaciones || {})[p.ticker] || null;
      const li = crear('li');
      ...
      if (p.bloque) texto.append(crear('small', p.bloque.replace(/-/g, ' '), 'bloque'));
      li.append(texto);
      if (dato) {
    ...
    // Peso por bloque DENTRO de cada divisa (no se suman USD y EUR, no se convierte). Reutiliza
    // las cotizaciones que ya se pidieron para el total: una sola llamada al servidor de precios.
    const pesos = window.CerebroCartera.pesosBloque(posiciones, r.cotizaciones || {});
    pintaPesos(pesos);
  ...
  const pintarNewsletter = async () => {
    ...
    const lista = await CerebroFuente.pide('boletines').catch(() => ({ ediciones: [], traza: 'no se pudo cargar el indice de ediciones' }));
  ...
  const cerrarNota = () => { $('#biblioteca-lectura').hidden = true; $('#biblioteca-indice').hidden = false; };
  ...
  const iniciar = () => {
    ...
    if (formCartera) {
      formCartera.addEventListener('submit', (e) => {
        e.preventDefault();
        const ticker = $('#cartera-ticker').value.trim().toUpperCase();
        if (!ticker) return;
        const posiciones = leerCartera();
        const nueva = {
          ticker,
